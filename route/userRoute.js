...
router.get("/all-approved-news", async (req, res) => {
   try {
      const result = await NewsModel.find({ isApproved: true }).sort({ createdAt: -1 });
      res.json({
         code: 200,
         message: "Data fetched succeessfully..",
         data: result
...
router.get("/admin-all-list", async (req, res) => {
   try {
      const result = await NewsModel.find().sort({ createdAt: -1 });
      res.json({
         code: 200,
         message: "Data fetched succeessfully..",
         data: result
...
router.get('/top-ten-news', async (req, res) => {
   try {
      const result = await NewsModel.find({ type: "image", isApproved: true }).sort({ createdAt: -1 }).limit(10);
      res.json({
         code: 200,
...
router.get('/top-category', async (req, res) => {
   try {
      const result = await NewsModel.find({ type: "image", isApproved: true }).sort({ createdAt: -1 })
      const seen = new Set();
...
router.get('/top-video', async (req, res) => {
   try {
      const result = await NewsModel.find({ type: "video", isApproved: true }).sort({ createdAt: -1 }); 
